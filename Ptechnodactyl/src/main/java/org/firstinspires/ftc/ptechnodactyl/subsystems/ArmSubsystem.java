...
public class ArmSubsystem implements Subsystem, Loggable {
    ...
    public static class ArmConstants {
        ...
        public static double getSlideTorque(double angle) {
            return Math.sin(angle + WRIST_ASM_OFFSET.angle()) * WRIST_ASM_KG * FORCE_GRAVITY / SLIDE_RAD_TO_M;
        }
        ...
        public static double getArmTorque(double angle, double slidePos) {
            ...
            return (wrist.getX() * WRIST_ASM_KG + slide.getX() * SLIDE_MOTOR_KG) * FORCE_GRAVITY / PITCH_RAD_TO_RAD;
        ...
        public static PIDCoefficients slidePID = new PIDCoefficients(3.0, 0.2, 0.0);
    ...
    public ArmSubsystem(EncodedMotor<DcMotorEx> pitchMotor, EncodedMotor<DcMotorEx> slideMotor, Servo wristServo) {
        ...
        pitchPidController = new PIDFController(
                ...
                (ticks, velocity) -> {
                    ...
                    double desiredCurrent = ArmConstants.getArmTorque(angle, slidePos) / ArmConstants.KT ;

                    return (desiredCurrent * ArmConstants.R + ArmConstants.KV * velocity / ArmConstants.ENCODER_TICKS_TO_RAD) / 12;
   ...
   public void setSlidePos(double slidePos) {
    ...
    public double getArmAngleRad() {
        ...
        // subtract it
        return pitchMotor.getCurrentPosition() / ArmConstants.ENCODER_TICKS_TO_RAD / ArmConstants.PITCH_RAD_TO_RAD - pitchOffset;
    }

    public double getArmAngleVelRadS(){
        return pitchMotor.getVelocity() / ArmConstants.ENCODER_TICKS_TO_RAD / ArmConstants.PITCH_RAD_TO_RAD;

    }


    public double getSlidePositionM() {
        return slideMotor.getCurrentPosition() / ArmConstants.ENCODER_TICKS_TO_RAD / ArmConstants.SLIDE_RAD_TO_M - slideOffset;
    }

    public double getSlideVelMS(){
        return slideMotor.getVelocity() / ArmConstants.ENCODER_TICKS_TO_RAD / ArmConstants.SLIDE_RAD_TO_M;
